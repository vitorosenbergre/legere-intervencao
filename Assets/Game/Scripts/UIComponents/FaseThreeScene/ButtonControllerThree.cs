...
public class ButtonControllerThree : MonoBehaviour
{
    ...
    IEnumerator WaitWinFaseOne()
    {
        ...
        PlayerController.faseController = "three";

        Starter();

        //SceneManager.LoadScene("Four_SC");
        //Time.timeScale = 1f;
    }
    ...
    public void LoadScenePartOne()
    {
        ...
        int randomNumber = sourceImage.pickRandomNumber(74);     //pegar vogal
        int randomDecision = sourceImage.pickRandomNumber(4);   // pegar botao
        correctButton = randomDecision;

        actualAudio = sourceImage.pickStringFaseThree(randomNumber); // pegou uma das vogais
        soundSource.clip = Resources.Load<AudioClip>("Audio/Fase3/silabas/" + actualAudio); // pegou o audio 
        ...
        while (manager)
        {
            int randomNumberVogals = sourceImage.pickRandomNumber(74);
        ...
        for(int i=0; i < 3; i++)
        {
            ...
            {
                case 0:
                    firstButtonText.text = sourceImage.pickStringFaseThree(NVs[i + 1]).ToUpper();
                    break;
                case 1:
                    secondButtonText.text = sourceImage.pickStringFaseThree(NVs[i + 1]).ToUpper();
                    break;
                case 2:
                    thirdButtonText.text = sourceImage.pickStringFaseThree(NVs[i + 1]).ToUpper();
                    break;
                case 3:
                    fourthButtonText.text = sourceImage.pickStringFaseThree(NVs[i + 1]).ToUpper();
                    break;
            }
        }
    ...
    {
        SceneManager.LoadScene("Three_SC");
        Time.timeScale = 1f;
    }
