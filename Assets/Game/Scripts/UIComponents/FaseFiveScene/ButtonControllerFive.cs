...
public class ButtonControllerFive : MonoBehaviour
{
    ...
    IEnumerator WaitWinFaseOne()
    {
        ...
        // aqui que vou fazer mudar pra Fase 3
        PlayerController.faseController = "five";
    ...
    public void LoadScenePartOne()
    {
        ...
        int randomNumber = sourceImage.pickRandomNumber(247);     //pegar vogal
        int randomDecision = sourceImage.pickRandomNumber(4);   // pegar botao
        correctButton = randomDecision;

        actualAudio = sourceImage.pickStringFaseFive(randomNumber); // pegou uma das vogais
        soundSource.clip = Resources.Load<AudioClip>("Audio/Fase5/silabas/" + actualAudio); // pegou o audio 
        ...
        while (manager)
        {
            int randomNumberVogals = sourceImage.pickRandomNumber(247);
        ...
        for(int i=0; i < 3; i++)
        {
            ...
            {
                case 0:
                    firstButtonText.text = sourceImage.pickStringFaseFive(NVs[i + 1]).ToUpper();
                    break;
                case 1:
                    secondButtonText.text = sourceImage.pickStringFaseFive(NVs[i + 1]).ToUpper();
                    break;
                case 2:
                    thirdButtonText.text = sourceImage.pickStringFaseFive(NVs[i + 1]).ToUpper();
                    break;
                case 3:
                    fourthButtonText.text = sourceImage.pickStringFaseFive(NVs[i + 1]).ToUpper();
                    break;
            }
        }
    ...
    {
        SceneManager.LoadScene("Five_SC");
        Time.timeScale = 1f;
    }
